...
const gamesData = [
  {
    ...
    cover:
      "https://img.gamemonetize.com/qsd2h427rg6z6gza2xekqsdwgk1dwksa/512x384.jpg", // 使用线上
    videoBackground:
      "https://gamemonetize.video/video/qsd2h427rg6z6gza2xekqsdwgk1dwksa-1682287965.mp4", // 添加视频背景
    genre: ["action", "adventure"],
    platform: ["mobile", "pc"],
    rating: "5-star",
    releaseDate: "2025-03-27",
    description:
      "Embark on an epic adventure in Stickman Ghost Online, where you'll transform into a powerful stickman warrior fighting against dark forces. This action-packed adventure game combines intense combat with strategic gameplay as you battle through various levels filled with challenging enemies.",
    features: [
      "Dynamic combat system",
      "Collect coins & upgrade",
      "PC & mobile support",
      "Multiple levels",
    ],
...
function addStructuredData() {
  const structuredData = {
    ...
    description:
      "Discover and download free games across all platforms. Browse games by genre, platform, and user ratings.",
  };
...
function initThreeJsBackground() {
  ...
  // 创建文字材质
  const textMaterials = textColors.map(
    (color) =>
  ...
  fontLoader.load(
    ...
    function (font) {
      ...
      // 创建"GAMES"文字
      const gamesMesh = createTextMesh(
        font,
        "GAMES",
        textMaterials[2],
        5,
  ...
  for (let i = 0; i < symbolsCount; i++) {
    // 随机选择几何体
    const geometry =
      symbolGeometries[Math.floor(Math.random() * symbolGeometries.length)];
  ...
  function animate() {
    ...
    if (textGroup && textGroup.children.length > 0) {
      ...
      // 决定是否应该切换淡入/淡出状态
      if (
        textAnimState.fadeDirection > 0 &&
        textAnimState.timer - textAnimState.lastSwitch >
          textAnimState.showDuration
      ) {
      ...
      const fadeSpeed =
        textAnimState.fadeDirection > 0
          ? textAnimState.fadeInSpeed
          : textAnimState.fadeSpeed;
      textAnimState.fadeProgress +=
        textAnimState.fadeDirection * fadeSpeed * deltaTime;
      textAnimState.fadeProgress = Math.max(
        0,
        Math.min(1, textAnimState.fadeProgress)
      );
      ...
      textGroup.children.forEach((child, index) => {
        ...
        const pulseScale =
          baseScale + Math.sin(time * 0.8 + index * 0.6) * 0.05 * easedFade;
        ...
        if (textAnimState.fadeDirection < 0) {
          // 淡出时稍微旋转并缩小
          child.rotation.y =
            Math.sin(time * 0.4 + index) * 0.1 + (1 - easedFade) * 0.2;
          child.position.z = (1 - easedFade) * -2;
        } else {
          // 淡入时从侧面旋转进入
          child.rotation.y =
            Math.sin(time * 0.4 + index) * 0.1 * easedFade +
            (1 - easedFade) * -0.5;
        ...
        <div class="filter-container">
            <div class="filter-group">
                ...
                <select id="genre-filter">
                    <option value="" ${
                      currentFilters.genre === "" ? "selected" : ""
                    }>All</option>
                    <option value="3d" ${
                      currentFilters.genre === "3d" ? "selected" : ""
                    }>3D</option>
                    <option value="action" ${
                      currentFilters.genre === "action" ? "selected" : ""
                    }>Action</option>
                    <option value="adventure" ${
                      currentFilters.genre === "adventure" ? "selected" : ""
                    }>Adventure</option>
                    <option value="arcade" ${
                      currentFilters.genre === "arcade" ? "selected" : ""
                    }>Arcade</option>
                    <option value="baby" ${
                      currentFilters.genre === "baby" ? "selected" : ""
                    }>Baby</option>
                    <option value="bejeweled" ${
                      currentFilters.genre === "bejeweled" ? "selected" : ""
                    }>Bejeweled</option>
                    <option value="boys" ${
                      currentFilters.genre === "boys" ? "selected" : ""
                    }>Boys</option>
                    <option value="clicker" ${
                      currentFilters.genre === "clicker" ? "selected" : ""
                    }>Clicker</option>
                    <option value="cooking" ${
                      currentFilters.genre === "cooking" ? "selected" : ""
                    }>Cooking</option>
                    <option value="girls" ${
                      currentFilters.genre === "girls" ? "selected" : ""
                    }>Girls</option>
                    <option value="haze" ${
                      currentFilters.genre === "haze" ? "selected" : ""
                    }>Haze</option>
                    <option value="hypercasual" ${
                      currentFilters.genre === "hypercasual" ? "selected" : ""
                    }>Hypercasual</option>
                    <option value="io" ${
                      currentFilters.genre === "io" ? "selected" : ""
                    }>IO</option>
                    <option value="multiplayer" ${
                      currentFilters.genre === "multiplayer" ? "selected" : ""
                    }>Multiplayer</option>
                    <option value="player" ${
                      currentFilters.genre === "player" ? "selected" : ""
                    }>Player</option>
                    <option value="puzzle" ${
                      currentFilters.genre === "puzzle" ? "selected" : ""
                    }>Puzzle</option>
                    <option value="racing" ${
                      currentFilters.genre === "racing" ? "selected" : ""
                    }>Racing</option>
                    <option value="shooting" ${
                      currentFilters.genre === "shooting" ? "selected" : ""
                    }>Shooting</option>
                    <option value="soccer" ${
                      currentFilters.genre === "soccer" ? "selected" : ""
                    }>Soccer</option>
                    <option value="sports" ${
                      currentFilters.genre === "sports" ? "selected" : ""
                    }>Sports</option>
                    <option value="stickman" ${
                      currentFilters.genre === "stickman" ? "selected" : ""
                    }>Stickman</option>
                </select>
            </div>
            <div class="filter-group">
                ...
                <select id="platform-filter">
                    <option value="" ${
                      currentFilters.platform === "" ? "selected" : ""
                    }>All</option>
                    <option value="pc" ${
                      currentFilters.platform === "pc" ? "selected" : ""
                    }>PC</option>
                    <option value="mobile" ${
                      currentFilters.platform === "mobile" ? "selected" : ""
                    }>Mobile</option>
                </select>
            </div>
            <div class="filter-group">
                ...
                <select id="rating-filter">
                    <option value="" ${
                      currentFilters.rating === "" ? "selected" : ""
                    }>All</option>
                    <option value="1-star" ${
                      currentFilters.rating === "1-star" ? "selected" : ""
                    }>★</option>
                    <option value="2-star" ${
                      currentFilters.rating === "2-star" ? "selected" : ""
                    }>★★</option>
                    <option value="3-star" ${
                      currentFilters.rating === "3-star" ? "selected" : ""
                    }>★★★</option>
                    <option value="4-star" ${
                      currentFilters.rating === "4-star" ? "selected" : ""
                    }>★★★★</option>
                    <option value="5-star" ${
                      currentFilters.rating === "5-star" ? "selected" : ""
                    }>★★★★★</option>
                </select>
            </div>
            <div class="filter-group">
                ...
                <select id="release-filter">
                    <option value="" ${
                      currentFilters.release === "" ? "selected" : ""
                    }>All</option>
                    <option value="new" ${
                      currentFilters.release === "new" ? "selected" : ""
                    }>Latest</option>
                    <option value="month" ${
                      currentFilters.release === "month" ? "selected" : ""
                    }>30 Days</option>
                    <option value="year" ${
                      currentFilters.release === "year" ? "selected" : ""
                    }>This Year</option>
                </select>
            </div>
        </div>
    ...
    gameCard.innerHTML = `
            <div class="game-card-inner">
                ...
                <div class="game-card-back">
                    ...
                    <!-- 发布日期（所有卡片都显示） -->
                    <p class="card-release-date"><small>发布日期: ${formatDate(
                      game.releaseDate
                    )}</small></p>
                    ...
                    <div class="game-features">
                        ...
                        ${
                          game.features.length > 2
                            ? "<span>+" + (game.features.length - 2) + "</span>"
                            : ""
                        }
                    </div>
                    `
                    ...
                    <div class="game-features">
                        ...
                        ${
                          game.features.length > 4
                            ? "<span>+" + (game.features.length - 4) + "</span>"
                            : ""
                        }
                    </div>
                    `
...
function filterGames() {
  ...
  if (genreFilter) {
    filteredGames = filteredGames.filter((game) =>
      game.genre.includes(genreFilter)
    );
  }
  ...
  if (ratingFilter) {
    filteredGames = filteredGames.filter((game) =>
      game.rating.includes(ratingFilter)
    );
  }
  ...
  if (releaseFilter) {
    ...
    if (releaseFilter === "new") {
      ...
      filteredGames = filteredGames.filter(
        (game) => new Date(game.releaseDate) >= oneMonthAgo
      );
    } else if (releaseFilter === "year") {
      const oneYearAgo = new Date(now.setFullYear(now.getFullYear() - 1));
      filteredGames = filteredGames.filter(
        (game) => new Date(game.releaseDate) >= oneYearAgo
      );
    }
  }
...
function handleVideoErrors() {
  const videos = document.querySelectorAll(
    ".video-background, .back-video-background"
  );
